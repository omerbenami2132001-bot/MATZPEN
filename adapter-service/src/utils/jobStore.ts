  ...
  toResponse(requestId: string): Record<string, unknown> | null {
    ...
    return {
      ...
      ...(job.status !== JOB_STATUS.RUNNING ? {
        results: {
          succeeded: job.results.filter((r) => r.success).map((r) => r.fileId),
          failed: job.results.filter((r) => !r.success).map((r) => ({
            fileId: r.fileId,
            failedStep: r.failedStep,
            errorType: r.errorType,
            error: r.error,
            ...(r.httpStatus ? { httpStatus: r.httpStatus, statusText: r.statusText } : {}),
          })),
        },
      } : {}),
    };
